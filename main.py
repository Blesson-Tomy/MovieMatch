...
Jsoncov = "please convert the given content into a json table with keys as numbers and each key has a Name, Year, Genre and Languages. Expand the body with valid information regarding the heading. You are to display the JSON output of each movie in a new line with no numberings and no additional brackets."
...
def get_json(content, prompt):
    jsonrec = model.invoke(content + prompt)
